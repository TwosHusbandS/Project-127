		...
		*/
        ...
        public static bool ChangeZIPExtractionPath(string pNewZIPPath)
        {
            ...
            if (HelperClasses.FileHandling.doesPathExist(pNewZIPPath) && pNewZIPPath.TrimEnd('\\') != Settings.ZIPExtractionPath.TrimEnd('\\'))
            {
                ...
                PopupWrapper.PopupProgress(PopupProgress.ProgressTypes.FileOperation, "Moving ZIP File Location", MyFileOperations);
        ...
        public static void SetDefaultEnableCopyingHardlinking()
        {
            ...
            else
            {
                ...
                {
                    yesno = PopupWrapper.PopupYesNo("It is recommended to use the slow but stable way for Upgrading / Downgrading.\nDo you want to do that?");
                }
                else
                {
                    yesno = PopupWrapper.PopupYesNo("It is recommended to NOT use the slow but stable way for Upgrading / Downgrading,\nsince its faster.\n\nDo you want to do that?\n\nClick No if you changed this because something didnt work.");
                }
                if (yesno == true)
                {
        ...
        public static string P127Mode
        {
            ...
            set
            {
                if (value != P127Mode)
                {
                    ...
                    if (!Popups.PopupMode.DirtyNoUpdate)
                    {
                        Globals.CheckForUpdate();
                        Auth.DynamicMTLOffsets.SetUpMTLOffsets();
                    }
                }
            }
        }
        ...
        public static string DragonEmuGameVersion
        {
            ...
            set
            {
                if (value != DragonEmuGameVersion)
                {
                    ...
                    {
                        PopupWrapper.PopupOk("Setting was not changed.");
                        return;
                    }
                }
            }
        ...
        public static bool EnableReturningPlayer
        {
            get
            {
                return GetBoolFromString(GetSetting("EnableReturningPlayer"));
            }
            set
            {
                ...
                if (value != Retailer)
                {
                    if (LauncherLogic.LaunchWay == LauncherLogic.LaunchWays.SocialClubLaunch)
                    {
                        if (ComponentManager.RecommendUpgradedGTA())
                        {
                            ...
                            if (!ComponentManager.CheckIfRequiredComponentsAreInstalled(true))
                            {
                                //PopupWrapper.PopupOk("Retailer was changed,\nbut you will need to download the Componenets later.").ShowDialog();
                                return;
                            }
                        }
                        else
                        {
                            //PopupWrapper.PopupOk("Retailer was not changed.").ShowDialog();
                            return;
                        }
        ...
        public static Keys GetKeyFromString(string pString)
        {
            ...
            {
                PopupWrapper.PopupError("Something broke while getting the Hotkeys from the Settings.\n Try Resetting Settings");
        ...
        public static int GetIntFromString(string pString)
        {
            ...
            {
                PopupWrapper.PopupError("Something broke while getting the a Number from the Settings.\n Try Resetting Settings");
            }
            return rtrn;
        }
        ...
        public static System.Drawing.Color GetColorFromString(string pColor)
        {
            ...
            {
                PopupWrapper.PopupError("Something broke while getting the Colors from the Settings.\n Try Resetting Settings");
            }
            return rtrn;
        }
